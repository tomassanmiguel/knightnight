...
public class SirDance : MonoBehaviour {
    ...
    [SerializeField]
    public KeyCode dashButton;                  //dash button
    [SerializeField]
    public float dashSpeed;                     //dash speed
    [SerializeField]
    public float dashDistance;                  //dash distance

    private bool aiming = false;
    private bool _hasJavelin = true;
    private float _groundY;
    private float _vSpeed;
    private bool _canDash = true;               //1 dash per bout; initially available and refreshes when knights turn around
    ...
    void Start()
    {
        ...
        if (combatant == null)
        {
            Debug.Log("Sir Dance requires Combatant component");
        }
    }
    ...
    void Update()
    {
        ...
        //Dash Logic
        if (Input.GetKeyDown(this.dashButton) && _canDash){             //if the dash button is pressed, then dash
            StartCoroutine("dash");
            _canDash = false;
        }
    }
    ...
    IEnumerator TurnAround()
    {
        ...
        if (facingLeft)
        {
            ...
            _hasJavelin = true;
            _canDash = true;                                    //Sir Dance can dash again!
            StartCoroutine("TrotLeft");
        }
        else
        {
    ...
    IEnumerator dash() {                                 //Dash function
        ...
        else {      //Sir Dance is facing right
            float endDashPos = transform.position.x + dashDistance;
            while (transform.position.x < endDashPos) {
